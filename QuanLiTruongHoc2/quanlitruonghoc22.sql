CREATE DATABASE QUANLITRUONGHOC2;
USE QUANLITRUONGHOC2;
DROP database QUANLITRUONGHOC2;
-- TẠO BẢNH GV
...
select * from lop;
-- Những học sinh có giới tính bằng nam
select * from hoc_sinh where gioi_tinh = 'Nam';
-- Những học sinh chưa có tên phụ huynh
select * from hoc_sinh where gioi_tinh is null;
...
-- Những học sinh có giới tính là nữ ở thanh khê
select * from hoc_sinh where gioi_tinh = 'Nữ' AND dia_chi = 'Thanh Khê';
-- Những học sinh nam ở địa chỉ hải châu và Những hs nữ ở địa chỉ thanh khê
select * from hoc_sinh where (gioi_tinh = 'Nam' AND dia_chi = 'Hải Châu') or (gioi_tinh = 'Nữ' AND dia_chi = 'Thanh Khê');
-- Những học sinh nam chưa có tên ph và những học sinh nữ chưa có lớp
select * from hoc_sinh where (gioi_tinh = 'Nam' AND ho_ten_ph is null) or (gioi_tinh = 'Nữ' AND ma_lop is null);
-- Mã môn học của những môn học được dạy ở học kì 2
select ma_mh from phu_trach_bo_mon where hoc_ky = 'Học kỳ 2';

-- Luyện tập về LIKE
-- Những học sinh có họ tên bắt đầu bằng từ nguyễn
select * from hoc_sinh where ho_ten_hs Like  'Nguyễn%';
--  Những học sinh có họ tên kết thúc bằng từ nở
select * from hoc_sinh where ho_ten_hs Like '%Nở';
-- Những học sinh có họ tên bằng Thị
select * from hoc_sinh where ho_ten_hs Like '%Thị%';
-- Những học sinh chứa từ thị ở giữa
select ho_ten_hs from hoc_sinh where ho_ten_hs Like '%Thị%' AND ho_ten_hs not like 'Thị%' AND ho_ten_hs not Like '%Thị';
-- Những học sinh có họ tên với độ dài là 15 kí tự (kể cả khoảng trống)
select ho_ten_hs from hoc_sinh where  char_length(ho_ten_hs) = 15;
-- Những học sinh có họ tên và độ dài tối đa là 12 kí tựa
select ho_ten_hs from hoc_sinh where  length(ho_ten_hs) = 12;
