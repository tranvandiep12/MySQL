...
-- học sinh chưa từng thi môn nào.
SELECT ho_ten_hs
FROM hoc_sinh
WHERE ma_hs NOT IN (
...
-- Giáo viên chưa từng phụ trách môn học nào
SELECT ho_ten_gv
FROM giao_vien
WHERE ma_gv NOT IN (
    SELECT DISTINCT ma_gvpt 
    FROM phu_trach_bo_mon
);
-- Giáo viên chưa từng chủ nhiệm lớp nào
SELECT ho_ten_gv
FROM giao_vien
WHERE ma_gv NOT IN (
...
-- Môn học chưa từng được tổ chức thi lần nào
SELECT ten_mh
FROM mon_hoc
WHERE ma_mh NOT IN (
...
-- Đếm xem tương ứng với mỗi địa chỉ, số lượng học sinh đang ở mỗi địa chỉ là bao nhiêu em. Chỉ hiển thị kết quả cho những địa chỉ có ít nhất 5 học sinh đang ở đó.
