...
--  Liệt kê họ tên học sinh, giới tính, địa chỉ của các nhóm được phân nhóm theo họ tên học sinh và giới tính trong bảng "học_sinh". Thử suy nghĩ về nguyên nhân lỗi nếu có lỗi xảy ra (*)
SELECT ho_ten_hs, gioi_tinh, dia_chi
FROM hoc_sinh
GROUP BY ho_ten_hs, gioi_tinh;
--  Đếm số lượng học sinh là nam.
SELECT COUNT(*) AS soLuongNam
FROM hoc_sinh
WHERE gioi_tinh = 'Nam';
-- Đếm số lượng học sinh trong lớp có tên là Lớp 1A và lớp đó nằm trong năm học 2022-2023.
SELECT COUNT(*) AS soLuongHocSinh
FROM hoc_sinh
WHERE lop = 'Lớp 1A' AND nam_hoc = '2022-2023';

--  Đếm những môn học đã từng có học sinh thi giữa kỳ được 9 điểm trở lên (cách 1 - sử dụng WHERE).
...
-- Đếm số lượng học sinh đang ở mỗi địa chỉ (của học sinh).
SELECT dia_chi, COUNT(*) AS soLuongHocSinh
FROM hoc_sinh
GROUP BY dia_chi;

-- Liệt kê điểm thi cao nhất của từng môn học (dựa vào điểm thi cuối kỳ).
...
JOIN ket_qua_hoc_tap kq ON mh.ma_mh = kq.ma_mh
GROUP BY mh.ten_mh;
-- Liệt kê điểm thi trung bình của từng môn học (dựa vào điểm thi cuối kỳ).
