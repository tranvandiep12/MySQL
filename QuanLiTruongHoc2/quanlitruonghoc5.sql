...
-- Họ tên của toàn bộ học sinh trong trương, nếu họ tên trùng nhau thì chỉ hiển thị 1 lần
select distinct ho_ten_hs from hoc_sinh;
-- Mã lơp của các lớp đã có học sinh học ở đó . Nếu mã lớp nào trùng nhau thì chỉ hiển thị một lần 
select distinct ma_lop , ho_ten_hs from hoc_sinh;
-- Mã môn học của những môn học đã có it nhất 1 giáo viên được phân công phụ trách 
select distinct ma_mh from phu_trach_bo_mon where ma_mh is not null;
-- Mã môn học của những môn học đã từng được tổ chức thi it nhất 1 lần 
select distinct ma_mh from ket_qua_hoc_tap;
-- Liệt kê mã giáo viên của những giáo viên đã từng làm chủ nhiệm cho it nhất 1 lớp nào đó 
