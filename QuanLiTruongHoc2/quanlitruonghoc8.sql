...
-- Liệt kê những địa chỉ khác nhau trong bảng hoc_sinh
select distinct dia_chi from hoc_sinh;
-- Liệt kê ho_ten_hs, gioi_tinh của các nhóm được phân nhóm theo ho_ten_hs và gioi_tinh trong bang hoc_sinh
select ho_ten_hs, gioi_tinh from hoc_sinh group by ho_ten_hs, gioi_tinh;
-- Liệt kê ho_ten_hs của các nhóm được phân nhóm theo ho_ten_hs và gioi_tinh trong bang hoc_sinh
select ho_ten_hs from hoc_sinh group by ho_ten_hs;
-- Liệt kê ma_mh, ten_mh, diem_thi_cuoi_ky của từng môn học chia theo từng mức điểm thi cuối kỳ.
...
-- Liệt kê ma_gv, ten_gv của những giáo viên đã từng được phân công phụ trách ít nhất 1 môn học. Nếu ma_gv, ten_gv trùng lặp nhiều lần thì chỉ hiện thị 1 lần trong kết quả trả về.
SELECT DISTINCT gv.ma_gv, gv.ho_ten_gv 
FROM giao_vien gv 
JOIN phu_trach_bo_mon ptbm ON gv.ma_gv = ptbm.ma_gvpt;

-- Liệt kê tháng và năm của những tháng và năm đã có ít nhất 1 học sinh tham gia thi cuối kỳ (đã có điểm_thi_cuoi_ky rồi).
